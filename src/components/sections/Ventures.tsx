...
import { cn, getFirebaseImageUrl, isOptimizableImage } from "@/lib/utils";
...
interface VenturesProps {
  ...
  serverVentures?: any[]; // 👇 FIX: Accept data from server
}
...
export default function Ventures({ data, serverVentures = [] }: VenturesProps) {
  const [hoveredId, setHoveredId] = useState<string | null>(null);

  // 👇 FIX: Map the server data to include the icons instantly
  const dynamicVentures = serverVentures.map(v => ({
    ...v,
    icon: getBrandAssets(v.ventureSlug).icon,
    color: getBrandAssets(v.ventureSlug).color,
    size: v.size || "small",
    displayOnHome: v.displayOnHome ?? true
  }))
  .sort((a, b) => {
    // If no display order is set, default to 999 to push it to the bottom
  ...
  return (
    <section id="ventures" className="relative z-10 w-full bg-black py-20 px-6">
      <div className="max-w-7xl mx-auto">
        ...
        {/* 👇 FIX: Loading spinner removed! It maps instantly. */}
        <motion.div 
              ...
              >
                <div className="group relative h-full w-full bg-[#171717] border border-white/5 rounded-2xl overflow-hidden flex flex-col p-5 transition-all duration-500">
                  <div className="absolute inset-0 z-0">
                    {imageUrl && (
                      <Image src={imageUrl} alt={v.title} fill className="object-cover transition-transform duration-700 group-hover:scale-110" unoptimized={!isOptimizableImage(imageUrl)} />
                    )}
