...
export default function RegionalClient({ initialLocation }: { initialLocation: any }) {
      ...
      <div className="pt-32 pb-24 px-6 max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 lg:gap-24">
            ...
            <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }}>
              <div className="flex items-center gap-6 mb-8">
                {iconUrl && (
                  <div className="relative w-16 h-16 md:w-24 md:h-24 shrink-0 rounded-2xl overflow-hidden border border-white/10 shadow-2xl bg-white/5">
                    <Image src={iconUrl} alt={`${location.cityRegion} Icon`} fill className="object-cover transition-all duration-700" unoptimized={!isOptimizableImage(iconUrl)} />
                  </div>
                )}
                <div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-6 pt-12 border-t border-white/10">
                <div className="space-y-1">
                  ...
                  <p className={cn("text-xs font-bold uppercase", location.status === "Live" ? "text-green-500" : location.status === "Planned" ? "text-amber-500" : "text-blue-500")}>
                    {location.status}
                  </p>
                </div>
                <div className="space-y-1">
          ...
          <div className="lg:col-span-7 flex flex-col justify-center">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.2 }} className="space-y-10">
