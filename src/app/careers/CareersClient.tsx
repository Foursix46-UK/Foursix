...
import { ArrowUpRight, MapPin, Briefcase, Mail, Building2,Banknote } from "lucide-react";
...
// 👇 Accept the instant data from the Server
export default function CareersClient({ initialPageData, initialJobs }: { initialPageData: any, initialJobs: JobPosting[] }) {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-20 mb-32">
          ...
          <section className="lg:col-span-7">
            <h2 className="text-sm font-bold uppercase tracking-widest mb-10 flex items-center gap-3">
              Open Positions <span className="text-white/20">/ {jobs.length === 0 ? "-" : `0${jobs.length}`}</span>
            </h2>
  ...
  {/* 👇 ADDED SALARY DISPLAY HERE */}
  {job.salary && (
    <span className="flex items-center gap-1 text-white/60"><Banknote className="w-2.5 h-2.5" /> {job.salary}</span>
  )}
</div>
                        </div>
                    ...
                    <AccordionContent className="pb-8 pt-0">
                      <div className="space-y-10 max-w-2xl">
                        ...
                        {job.salary && (
      <div className="space-y-4">
        <h4 className="text-[10px] font-bold uppercase tracking-widest text-white/40">Compensation</h4>
        <div className="inline-flex items-center gap-3 text-sm text-white/90 bg-white/5 border border-white/10 rounded-xl px-5 py-3">
          <Banknote className="w-4 h-4 text-primary" /> 
          <span className="font-semibold tracking-wide">{job.salary}</span>
        </div>
      </div>
    )}
