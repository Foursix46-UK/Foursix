...
// 👇 Accept both initialPageData and initialVentures
export default function VenturesClient({ initialPageData, initialVentures }: { initialPageData: any, initialVentures: any[] }) {
  const pageData = initialPageData;
  
  const sortedVentures = [...initialVentures].sort((a, b) => {
    ...
    <main className="min-h-screen bg-black">
      ...
      <div className="pt-24">
        <header className="px-6 py-20 max-w-7xl mx-auto border-b border-white/5">
          ...
          <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="text-5xl md:text-6xl font-sans font-semibold uppercase tracking-tight leading-none mb-8">
            {pageData?.heroTitleMain || "OUR"}<br />
            <span className="text-white/40">{pageData?.heroTitleHighlight || "VENTURES"}</span>
          </motion.h1>
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }} className="text-lg text-white/50 max-w-2xl font-light leading-relaxed tracking-tight whitespace-pre-wrap">
        ...
        {/* 👇 The ONLY VenturesOverview component, now properly using sortedVentures */}
        <VenturesOverview initialVentures={sortedVentures} />
